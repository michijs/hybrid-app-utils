...
export abstract class FileManager {
  ...
  static showSaveFilePicker(
    ...
    if (FileManager.supportsBrowserSaveFilePicker)
      ...
      return new Promise<FileSystemFileHandle>((resolve, reject) => {
        ...
        if (window.HybridInterface) {
          ...
          window.HybridInterface?.showSaveFilePicker(
          ...
          const allAccept = options?.types
            ...
            .map((x) => {
              return Object.entries(x.accept!).map(([key, value]) => {
                return Array.isArray(value) && value.length > 0 ? value : key;
              });
            })
            .flat(2);
          const el = document.createElement("input") as HTMLInputElement;
          el.setAttribute("type", "file");
          if (allAccept) el.setAttribute("accept", allAccept.join(","));
            ...
            if (el?.files)
              resolve(
                Array.from(el.files).map(
                  (x) => new FakeFileSystemFileHandle(x, this.download),
                ),
              );
          };
          el.click();
